...
const chatContainer = document.getElementById('chat-container'); // تم تعديل هذا السطر
...
usernameForm.addEventListener('submit', (e) => {
  ...
  if (username) {
    ...
    socket.emit('set username', username); // تم تعديل هذا السطر
...
messageForm.addEventListener('submit', (e) => {
  e.preventDefault();
  const msg = messageInput.value.trim(); // تم تعديل هذا السطر
  if(msg){
...
socket.on('chat message', (data) => {
  ...
  messageElement.classList.add('message'); // تم تعديل هذا السطر
...
socket.on('system message', (data) => {
  const messageElement = document.createElement('div');
  // استخدام كلاس خاص لرسائل النظام // تم تعديل هذا السطر
...
socket.on('users', (users) => {
  usersList.innerHTML = '';
  users.forEach(user => {
    const li = document.createElement('li'); // تم تعديل هذا السطر
...
// 1. إنشاء الاتصال بالخادم بمجرد أن يصبح محتوى الصفحة جاهزًا
// تأكد من أن الخادم يعمل على هذا العنوان والمنفذ
const socket = io('http://192.168.43.150:3001');
